...
contract DANNYPresale is Ownable {
    ...
    constructor() {
        saleStatus = true;
        rate = 300000000000000000; // $1
        tokenAggregator[address(0)] = AggregatorV3Interface(0x0567F2323251f0Aab15c8dFb1967E4e8A7D42aeE); //bnbusd feed address
        levelRate = [1000];
        withoutBuyRef = false;

        setSaleToken(0x046b82988a7113FCAd568B7102c7b823f4411385, 2000000000000000000000000, true);
        ...
        treasuryWallet = 0xE482c134EbB4165AA4Fd3D4E2521285bec70A5fA;
    }
    ...
    modifier claimEnabled() {
        require(claimStatus == true, "Presale: is not enabled");
        _;
    }

    function setSaleToken(address _saleToken, uint256 _totalTokensforSale, bool _saleStatus) public onlyOwner {
    ...
    function setClaimStatus(bool _claim) public onlyOwner{
        claimStatus = _claim;
    }

    function setPayableTokens(address _tokens, uint _prices, address _aggregator) public onlyOwner {
        payableTokens[_tokens] = true;
        tokenPrices[_tokens] = _prices;
        tokenAggregator[_tokens] = AggregatorV3Interface(_aggregator);
    }

    function payableTokenStatus(address _token, bool _status) external onlyOwner {
    ...
    function buyToken(address _token, uint256 _amount, address _referrer) external payable saleEnabled {
        uint256 saleTokenAmt = _token != address(0) ? getTokenAmount(_token, _amount) : getTokenAmount(address(0), msg.value);
        require((totalTokensSold + saleTokenAmt) < totalTokensforSale, "Presale: Not enough tokens to be sale");
        ...
        if (_token != address(0)) {
            require(_amount > 0 , "amount must be greator than");
            IERC20(_token).safeTransferFrom(msg.sender, treasuryWallet, _amount);
        }
        else{
    ...
    function claim() public claimEnabled{
        uint256 totalPurchase = buyersAmount[msg.sender].amount;
        uint256 totalClaimed = buyersAmount[msg.sender].totalclaimed;
        uint256 withdrawAvalible = totalPurchase > totalClaimed ? totalPurchase - totalClaimed : 0;
        require(withdrawAvalible > 0 , "you not have any pending claim!");
        IERC20(saleToken).safeTransfer(msg.sender, withdrawAvalible);
        buyersAmount[msg.sender].totalclaimed += withdrawAvalible;
    }
    ...
    function payReferral(uint256 value) internal returns (uint256) {
        ...
        for (uint i; i < levelRate.length; i++) {
            ...
            if(IERC20(saleToken).balanceOf(address(this)) >= c){
                totalReferal = totalReferal.add(c);
                parentAccount.reward = parentAccount.reward.add(c);
                IERC20(saleToken).transfer(parent, c);
